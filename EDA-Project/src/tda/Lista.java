...
public class Lista<T> {
    ...
    public T iesimo(int pos) {
        ...
        if (pos >= 1 && pos <= longitud()) {
            ...
            throw new RuntimeException("ERROR: no es posible devolver un valor de una posicion no valida!");
        }
    ...
    public void eliminar(int pos) {
        if (pos >= 1 && pos <= longitud()) {
            ...
            throw new RuntimeException("ERROR: no es posible eliminar");
        }
    ...
    //NUEVO METODO PARA PREGUNTA 4//
    public void reemplazarValor(T elemento, int pos){
        Nodo<T> nuevoNodo = new Nodo(elemento, null);
        if (pos >= 1 && pos <= longitud() + 1) {
            if (pos == 1) {
                nuevoNodo.setSgteNodo(cabeza.getSgteNodo());// de esta forma se reemplazara a la cabeza ya que el nuevo se conectara con el sigueinte de la cabeza e ignorara esta
                cabeza = nuevoNodo;
            } else { // pos>1
                Nodo<T> aux = cabeza;
                int posActual = 1;
                ...
                nuevoNodo.setSgteNodo(aux.getSgteNodo().getSgteNodo()); // de esta forma se hace un salto extra para ignorar al nodo y reemplazarlo
                aux.setSgteNodo(nuevoNodo);
            }
        }        
    }
